...
<button
    class={cx('sticky top-0 z-10 w-full text-black shadow-2xl', className)}
    on:click={onClick(toggleOpen)}
    on:keydown={onKeydown(toggleOpen)}
>
    <div
        class={cx(
            'relative flex h-16 cursor-pointer select-none items-center justify-between bg-white px-2 pt-2 pb-4 md:px-4',
